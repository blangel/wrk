...
public final class Wrk {
    ...
    private static void ensureTrelloToken() {
        if (TrelloUtil.USR_TOKEN.isEmpty()) {
            ...
            Output.print("  Wrk's key to use when obtaining your token is ^b^%s^r^", TrelloUtil.APP_DEV_KEY);
            Output.print("  For example, to give wrk read/write access; use url:");
            Output.print("    ^b^https://trello.com/1/authorize?key=%s&name=Wrk&expiration=never&response_type=token&scope=read,write^r^", TrelloUtil.APP_DEV_KEY);
